...
import { User, MapPin, Rocket, Globe, Heart } from 'lucide-react'
...
export default function UserProfilePopup() {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (stored) {
          ...
          if (!parsed.state?.userProfile || typeof parsed.state.userProfile.hasSeenWelcome !== 'boolean') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (name.trim()) {
      // Use the atomic method to set both profile and welcome flag together
      store.setUserProfileAndWelcome(name.trim(), location.trim())
    } else {
      // If no name provided, just set the welcome flag
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
              ...
              <button
                type="submit"
                className="flex-1 bg-gradient-to-r from-blue-500 to-purple-600 text-white py-3 px-4 rounded-xl hover:shadow-lg hover:shadow-blue-500/25 transition-all font-medium"
              >
                Get Started
              </button>
            </motion.div>
          ...
          >
            <div className="flex items-center gap-2 text-sm text-blue-700 dark:text-blue-300">
              ...
              <span>Your name will appear on the leaderboard when you complete typing tests!</span>
            </div>
          </motion.div>
        </motion.div>
      </motion.div>
